//! Chunking strategy: BASELINE (current)
//! Same-sender consecutive messages within 5-minute gap threshold.
//! This is the existing chunker for comparison.
...
fn main() {
    ...
    eprintln!("[baseline] Fetched {} messages", messages.len());

    // Use the existing chunker
    let chunks = app_lib::embeddings::chunker::chunk_messages(&messages);
    eprintln!("[baseline] Created {} chunks", chunks.len());

    // Build chunk texts
    let chunk_texts: Vec<String> = chunks
        .iter()
        .map(|c| c.concatenated_text.clone())
        .collect();
    ...
    // Compute stats
    let non_empty: Vec<&String> = chunk_texts.iter().filter(|t| !t.trim().is_empty()).collect();
    ...
    // Print header
    println!("=== BASELINE: Same-sender, 5min gap ===");
