//! Chunking strategy: FIXED-SIZE SLIDING WINDOWS
//! Groups N consecutive messages regardless of sender or timing.
//! Tests window sizes of 5, 10, and 20 messages.
//! Each chunk includes sender labels ("Me:", "Them:") for context.
...
/// Fixed-size sliding window with 50% overlap
fn chunk_sliding_window(
    messages: &[app_lib::db::models::Message],
    window_size: usize,
    step: usize,
) -> Vec<SlidingChunk> {
    let mut chunks: Vec<SlidingChunk> = Vec::new();
    let mut i = 0;

    while i < messages.len() {
        let end = (i + window_size).min(messages.len());
        let window = &messages[i..end];
        ...
        for msg in window {
            if let Some(ref t) = msg.text {
                if !t.trim().is_empty() {
                    ...
                    text_parts.push(format!("{}: {}", label, t));
                }
            }
        }
        ...
        if !text.is_empty() {
            chunks.push(SlidingChunk {
                text,
                first_rowid: window.first().unwrap().rowid,
                last_rowid: window.last().unwrap().rowid,
...
fn chunk_fixed_window(
    messages: &[app_lib::db::models::Message],
    window_size: usize,
) -> Vec<SlidingChunk> {
    chunk_sliding_window(messages, window_size, window_size)
}
...
fn main() {
    ...
    eprintln!("[sliding] Fetched {} messages", messages.len());
    ...
    // Test configurations: (label, window_size, step)
    let configs: Vec<(&str, usize, usize)> = vec![
        ("5msg-no-overlap", 5, 5),
        ("10msg-no-overlap", 10, 10),
        ("20msg-no-overlap", 20, 20),
        ("10msg-50pct-overlap", 10, 5),
    ];

    for (label, window_size, step) in &configs {
        eprintln!("[sliding] Testing {}...", label);
        let chunks = chunk_sliding_window(&messages, *window_size, *step);
        ...
        let over_300: usize = chunk_texts.iter().filter(|t| t.len() > 300).count();

        println!("=== SLIDING WINDOW: {} ===", label);
        ...
        println!("Chunks >300ch: {} ({:.1}%)", over_300, over_300 as f64 / chunks.len().max(1) as f64 * 100.0);
