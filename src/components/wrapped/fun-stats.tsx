  ...
  return (
    <Card className="card-glass">
      <CardHeader className="flex flex-row items-center gap-2">
        <Sparkles className="h-5 w-5 text-[#3B82C4] dark:text-yellow-400" />
      ...
      <CardContent>
        {isLoading && (
          <div className="flex items-center justify-center py-8">
            <div className="h-5 w-5 animate-spin rounded-full border-2 border-[#D1D5DB] dark:border-zinc-600 border-t-[#3B82C4] dark:border-t-yellow-400" />
          </div>
        )}
        ...
        {!isLoading && data && (
          <div className="space-y-6">
            ...
            <div className="text-center py-2">
              ...
              <p className="text-2xl font-bold tracking-tight text-[#1B2432] dark:text-white">
                {data.primaryType}
              </p>
              {data.secondaryType && (
                ...
                {data.traits.map((trait) => (
                  <div key={trait.name} className="space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-[#1B2432] dark:text-zinc-200">
                        {trait.name}
                      </span>
                      <span className="text-xs tabular-nums text-[#94A3B3] dark:text-zinc-500">
                        {Math.round(trait.score * 100)}%
                      </span>
                    </div>
                    <p className="text-[11px] text-[#94A3B3] dark:text-zinc-600">
                      {trait.description}
                    </p>
                    <div className="h-1 w-full rounded-full bg-[#D1D5DB]/30 dark:bg-white/[0.06]">
                      <div
                        className="h-full rounded-full bg-gradient-to-r from-[#3B82C4] to-[#B8CEE2] dark:from-amber-400 dark:to-orange-400 transition-all"
