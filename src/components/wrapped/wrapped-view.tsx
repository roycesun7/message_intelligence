...
function YearSelector() {
  ...
  return (
    <div className="flex items-center gap-2">
      <label className="text-sm text-[#4E5D6E] dark:text-zinc-400">Year:</label>
      <select
        value={year}
        onChange={(e) => setYear(Number(e.target.value))}
        className="h-8 rounded-full border border-[#D1D5DB] dark:border-white/[0.08] bg-white dark:bg-white/[0.06] px-4 text-sm text-[#1B2432] dark:text-zinc-200 outline-none transition-colors hover:bg-[#ECEEF2] dark:hover:bg-white/[0.1] focus:ring-2 focus:ring-[#4E5D6E]/40 dark:focus:ring-blue-500/40"
      >
...
const MONTH_COLORS_LIGHT = [
  ...
  "#B8CEE2", "#D1D5DB", "#1B2432", "#4E5D6E",
  "#3B82C4", "#94A3B3", "#B8CEE2", "#D1D5DB",
];
const MONTH_COLORS_DARK = [
  "#60A5FA", "#93C5FD", "#3B82C4", "#7DD3FC",
  "#BAE6FD", "#A5B4C4", "#60A5FA", "#93C5FD",
  "#3B82C4", "#7DD3FC", "#BAE6FD", "#A5B4C4",
];

const DAY_COLORS_LIGHT = [
  "#3B82C4", "#1B2432", "#4E5D6E", "#94A3B3",
  "#B8CEE2", "#D1D5DB", "#1B2432",
];
const DAY_COLORS_DARK = [
  "#60A5FA", "#93C5FD", "#3B82C4", "#7DD3FC",
  "#BAE6FD", "#A5B4C4", "#60A5FA",
];
...
const GRID_STROKE_LIGHT = "rgba(27, 36, 50, 0.06)";
const GRID_STROKE_DARK = "rgba(255, 255, 255, 0.08)";
const AXIS_TICK_LIGHT = { fill: "#94A3B3", fontSize: 11 };
const AXIS_TICK_DARK = { fill: "#71717A", fontSize: 11 };

// ── Theme-aware chart line/fill colors ───────────────────
const CHART_LINE = { light: { sent: "#1B2432", received: "#3B82C4" }, dark: { sent: "#60A5FA", received: "#93C5FD" } };
const CHART_BAR = { light: { my: "#1B2432", their: "#3B82C4" }, dark: { my: "#60A5FA", their: "#93C5FD" } };
...
function TemporalTrendsChart({ chatId, year }: { chatId: number; year: number }) {
  ...
  const tooltipStyle = isDark ? CHART_TOOLTIP_STYLE_DARK : CHART_TOOLTIP_STYLE;
  const lineColors = isDark ? CHART_LINE.dark : CHART_LINE.light;
          ...
          <AreaChart data={points} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
            <defs>
              <linearGradient id="gradientSent" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={lineColors.sent} stopOpacity={0.3} />
                <stop offset="100%" stopColor={lineColors.sent} stopOpacity={0.05} />
              </linearGradient>
              <linearGradient id="gradientReceived" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={lineColors.received} stopOpacity={0.3} />
                <stop offset="100%" stopColor={lineColors.received} stopOpacity={0.05} />
              </linearGradient>
            </defs>
...
function RelationshipMetrics({ chatId }: { chatId: number }) {
  ...
  const axisTick = isDark ? AXIS_TICK_DARK : AXIS_TICK_LIGHT;
  const tooltipStyle = isDark ? CHART_TOOLTIP_STYLE_DARK : CHART_TOOLTIP_STYLE;
  const barColors = isDark ? CHART_BAR.dark : CHART_BAR.light;
    ...
    return (
      <div className="mb-8 flex items-center justify-center py-12">
        <div className="flex flex-col items-center gap-3">
          <div className="h-6 w-6 animate-spin rounded-full border-2 border-[#D1D5DB] dark:border-zinc-600 border-t-[#4E5D6E] dark:border-t-blue-500" />
          <p className="text-sm text-[#94A3B3] dark:text-zinc-500">Loading relationship metrics...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mb-8 space-y-6">
      <p className="text-lg font-semibold text-[#1B2432] dark:text-white">Between You Two</p>
          ...
          <Card className="card-glass">
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="mt-3 space-y-1">
                    <p className="text-xs text-[#4E5D6E] dark:text-zinc-400">
                      Median:{" "}
                      <span className="text-[#1B2432] dark:text-zinc-200">
                ...
                {/* Them column */}
                <div>
                  <p className="mb-2 text-sm font-medium text-[#3B82C4] dark:text-purple-400">
                    Them
                  </p>
                  <p className="text-2xl font-bold text-[#1B2432] dark:text-white">
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-[#1B2432] dark:text-zinc-200">
...
export function WrappedView() {
  ...
  const axisTick = isDark ? AXIS_TICK_DARK : AXIS_TICK_LIGHT;
  const tooltipStyle = isDark ? CHART_TOOLTIP_STYLE_DARK : CHART_TOOLTIP_STYLE;
  const monthColors = isDark ? MONTH_COLORS_DARK : MONTH_COLORS_LIGHT;
  const dayColors = isDark ? DAY_COLORS_DARK : DAY_COLORS_LIGHT;
  const yearColors = isDark ? YEAR_COLORS_DARK : YEAR_COLORS_LIGHT;
    ...
    <div className="bg-[#ECEEF2] dark:bg-zinc-950 p-6 space-y-[25px]">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[#1B2432] dark:text-white">
            ...
            {isPerChat && (
              <span className="ml-2 text-2xl font-bold text-[#4E5D6E] dark:text-blue-400">
      ...
      <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
        ...
        <StatCard
          ...
          icon={Inbox}
          gradient="bg-gradient-to-br from-[#3B82C4] to-[#B8CEE2]"
        />
        {isPerChat ? (
          <StatCard
            title="Days Active"
                    ...
                    return (
                      <li key={c.key} className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between">
                            <span className="truncate text-sm text-[#1B2432] dark:text-zinc-200">
                              {name}
                            </span>
                          ...
                          <div className="mt-1 h-1 w-full rounded-full bg-[#D1D5DB]/30 dark:bg-white/[0.06]">
                            <div
                              className="h-full rounded-full bg-gradient-to-r from-[#3B82C4] to-[#64ACFF] dark:from-blue-400 dark:to-purple-400"
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                        </div>
                ...
                return (
                  <div className="space-y-3">
                    <div className="flex items-baseline gap-2">
                      <span className="text-3xl font-bold text-[#1B2432] dark:text-indigo-400">
                    ...
                    <div className="flex gap-4 text-sm">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[#1B2432] dark:text-zinc-200">{lnSent.toLocaleString()}</span>
                        <span className="text-[#94A3B3] dark:text-zinc-500">sent</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <div className="h-2.5 w-2.5 rounded-full bg-[#94A3B3] dark:bg-zinc-500" />
                        <span className="text-[#1B2432] dark:text-zinc-200">{lnReceived.toLocaleString()}</span>
                        <span className="text-[#94A3B3] dark:text-zinc-500">received</span>
                      </div>
                    </div>
                    {lnTotal > 0 && (
                      <div className="h-2 w-full rounded-full bg-[#D1D5DB]/30 dark:bg-white/[0.06] overflow-hidden">
                        <div
                  ...
                  return (
                    <li key={c.key} className="flex items-center justify-between">
                      <span className="flex items-center gap-2">
                        <span className="text-xs font-bold text-[#94A3B3] dark:text-zinc-500">{i + 1}</span>
                        <span className="truncate text-sm text-[#1B2432] dark:text-zinc-200">{name}</span>
                      </span>
                      <span className="text-xs text-[#4E5D6E] dark:text-zinc-400">{c.total.toLocaleString()}</span>
                    </li>
                  );
                })}
              </ol>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {topOpeners.map((o) => (
                <span
                  key={o.text}
                  className="inline-flex items-center gap-1.5 rounded-full bg-[#D1D5DB]/30 dark:bg-zinc-800 px-3 py-1.5 text-sm text-[#1B2432] dark:text-zinc-200"
                >
                  &ldquo;{o.text}&rdquo;
                  <span className="text-xs text-[#94A3B3] dark:text-zinc-500">x{o.count}</span>
                </span>
              ))}
            </div>
          </CardContent>
